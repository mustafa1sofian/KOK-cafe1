...
import { Utensils, Coffee, Wind, Loader2, AlertCircle, X } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="pt-20">
        ...
        <section className="py-8 md:py-12 bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 relative overflow-hidden">
          {/* Decorative pattern */}
          ...
          <div className="container mx-auto px-4 text-center relative z-10">
            <h1 className={`text-3xl md:text-4xl font-bold bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-400 bg-clip-text text-transparent mb-2 ${
        ...
        <section className="py-16">
          <div className="container mx-auto px-4">
            ...
            <div className="mb-6">
              ...
              <div className="flex gap-2 md:gap-3 overflow-x-auto pb-3 scrollbar-hide snap-x snap-mandatory" style={{ scrollBehavior: 'smooth' }}>
                {categories.map((category) => (
                  <Button
                    ...
                    className={`snap-center flex-shrink-0 min-w-[160px] md:min-w-[180px] h-12 md:h-14 ${
                      activeCategory === category.id 
                        ? 'bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-600 hover:to-yellow-600 text-white shadow-lg scale-105 border-0' 
                        : 'bg-white border-2 border-gray-200 text-gray-700 hover:border-amber-400 hover:bg-amber-50'
                    } ${isRTL ? 'font-arabic' : 'font-english'} transition-all duration-300 font-semibold rounded-xl`}
                  >
            ...
            {activeCategories && activeCategories.subcategories.length > 0 && (
              <div className="mb-8">
                <h3 className={`text-sm md:text-base font-semibold text-gray-600 mb-3 px-1 ${isRTL ? 'font-arabic text-right' : 'font-english text-left'}`}>
                ...
                <div className="flex gap-2 overflow-x-auto pb-3 scrollbar-hide snap-x snap-mandatory" style={{ scrollBehavior: 'smooth' }}>
                  {activeCategories.subcategories.map((subcategory) => (
                    <Button
                      ...
                      className={`snap-center flex-shrink-0 px-4 md:px-5 h-10 md:h-11 ${
                        activeSubcategory === subcategory.id 
                          ? 'bg-gray-900 hover:bg-gray-800 text-white shadow-md scale-105 border-0' 
                          : 'bg-white/80 backdrop-blur-sm border border-gray-300 text-gray-700 hover:bg-white hover:border-gray-900 hover:shadow-sm'
                      } ${isRTL ? 'font-arabic' : 'font-english'} transition-all duration-300 font-medium rounded-full text-sm`}
                    >
                    ...
                    {subcategory.items.length > 0 ? (
                      <div className="grid grid-cols-2 gap-3 md:gap-4">
                        {subcategory.items.map((item) => (
                          <Card 
                            key={item.id} 
                            className="group hover:shadow-xl transition-all duration-300 hover:scale-[1.02] border-0 overflow-hidden bg-white cursor-pointer"
                          ...
                          >
                            <div className="relative">
                              ...
                              <div className="aspect-square bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center overflow-hidden">
                                {item.imageUrl ? (
                                  ...
                                  <Utensils className="w-8 h-8 md:w-12 md:h-12 text-gray-400" />
                                )}
                              </div>
                              ...
                              {item.isFeatured && (
                                <div className="absolute top-2 left-2 rtl:left-auto rtl:right-2">
                                  <span className="bg-gradient-to-r from-amber-500 to-yellow-500 text-white px-2 py-1 rounded-full text-[10px] font-bold shadow-md flex items-center gap-1">
                            ...
                            <CardContent className="p-2.5 md:p-3">
                              ...
                              <h3 className={`text-sm md:text-base font-bold text-gray-900 mb-1.5 line-clamp-2 leading-tight min-h-[2.5rem] ${
                                isRTL ? 'font-arabic text-right' : 'font-english text-left'
                              }`}>
                              ...
                              <div className={`flex items-center gap-1 ${isRTL ? 'justify-end' : 'justify-start'}`}>
                                <span className="text-base md:text-lg font-bold bg-gradient-to-r from-amber-600 to-yellow-600 bg-clip-text text-transparent">
                                  {item.price}
                                </span>
      ...
      {isModalOpen && selectedItem && (
        ...
        >
          ...
          >
            ...
            <button
              onClick={closeModal}
              className="absolute top-4 right-4 rtl:right-auto rtl:left-4 z-10 w-10 h-10 flex items-center justify-center bg-white/90 backdrop-blur-sm rounded-full shadow-lg hover:bg-white transition-all duration-300 group"
            >
            ...
            <div className="overflow-y-auto max-h-[85vh] md:max-h-[90vh]">
              ...
              <div className="relative aspect-[4/3] bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden">
                ...
                {selectedItem.isFeatured && (
                  <div className="absolute top-4 left-4 rtl:left-auto rtl:right-4">
                    <span className="bg-gradient-to-r from-amber-500 to-yellow-500 text-white px-4 py-2 rounded-full text-sm font-bold shadow-lg flex items-center gap-2">
              ...
              <div className="p-5 md:p-6">
                ...
                <div className={`flex items-start justify-between gap-4 mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  ...
                  {/* Price Badge */}
                  <div className="flex-shrink-0 bg-gradient-to-br from-amber-50 to-yellow-50 px-4 py-3 rounded-xl border-2 border-amber-200">
                    <div className={`text-xs text-gray-600 mb-1 ${isRTL ? 'font-arabic text-right' : 'font-english text-left'}`}>
                ...
                <div>
                  <Button
                    onClick={closeModal}
                    className={`w-full h-12 bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-600 hover:to-yellow-600 text-white border-0 shadow-lg hover:shadow-xl transition-all duration-300 ${
                      isRTL ? 'font-arabic' : 'font-english'
                    }`}
                  >
