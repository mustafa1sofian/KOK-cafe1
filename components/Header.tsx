...
const Header = () => {
  ...
  return (
        ...
        <div className={`flex items-center justify-between py-3 md:py-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <nav className="hidden lg:flex items-center space-x-6 rtl:space-x-reverse">
            {navItems.map((item) => (
              <button
                ...
                className={`relative text-white/90 hover:text-yellow-400 transition-all duration-300 font-medium py-2 px-3 group text-sm ${
                  activeSection === item.key ? 'text-yellow-400' : ''
                }`}
              >
                {t(item.key)}
                {/* Active indicator */}
            ...
            >
              ...
              <span className="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-yellow-400 to-yellow-600 transform transition-transform duration-300 scale-x-0 group-hover:scale-x-100" />
            </button>
          </nav>
          ...
          <div className={`flex items-center space-x-4 rtl:space-x-reverse ${isRTL ? 'flex-row-reverse' : ''}`}>
            <Button
              ...
              className={`text-white/90 hover:text-yellow-400 hover:bg-white/10 transition-all duration-300 transform hover:scale-105 rounded-full px-3 py-2 text-sm ${
    ...
    <div className="fixed top-0 left-0 w-full h-0.5 bg-black/30 z-40">
      <div 
        className="h-full bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 transition-all duration-300 ease-out shadow-lg"
        style={{
